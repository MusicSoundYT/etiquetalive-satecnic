...
/** Confirma el código recibido por correo y activa el MFA (método email) de forma definitiva, abriendo sesión. */
export async function POST(req: NextRequest) {
  ...
  const { data: user } = await supabaseAdmin
    .from("users")
    .select("id, mfa_enabled, mfa_email_code_hash, mfa_email_code_expires_at")
    .eq("id", userId)
    .maybeSingle();

  if (!user || !user.mfa_email_code_hash) {
    return NextResponse.json({ error: "Solicita primero el código por correo." }, { status: 400 });
  }
  if (user.mfa_enabled) {
