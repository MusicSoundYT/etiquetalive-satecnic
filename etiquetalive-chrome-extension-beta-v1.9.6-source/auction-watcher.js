(() => {
  ...
  // Config dinámica: reconcile delay desde remoteConfig o storage
  let reconcileDelayMs = 6000; // default
  try {
    chrome.storage.local.get(["el_remote_config"], (r) => {
      if (r?.el_remote_config?.reconcileDelayMs) {
        reconcileDelayMs = Number(r.el_remote_config.reconcileDelayMs);
      }
    });
    chrome.storage.onChanged.addListener((changes, area) => {
      if (area === "local" && changes.el_remote_config?.newValue?.reconcileDelayMs) {
        reconcileDelayMs = Number(changes.el_remote_config.newValue.reconcileDelayMs);
      }
    });
  } catch (_) {}
  // También escuchar broadcast directo desde background
  try {
    chrome.runtime.onMessage.addListener((msg) => {
      if (msg?.type === "EL_CONFIG_UPDATE" && msg.reconcileDelayMs) {
        reconcileDelayMs = Number(msg.reconcileDelayMs);
      }
    });
  } catch (_) {}
  ...
  function extractPrice(text) {
    const s = String(text || "");
    let m = s.match(/[\p{L}\p{N}_.@\- ]{2,80}\s+ha\s+ganado\s*[:：-]\s*(\d{1,6}(?:[,.]\d{1,2})?)\s*(?:€|EUR)?/iu);
    if (m?.[1]) return norm(m[1]);
    // Actividad TikTok: "Rosa Granado Arroyo ha ganado el artículo de la subasta 4 €"
    m = s.match(/[\p{L}\p{N}_.@\- ]{2,80}\s+ha\s+ganado\s+el\s+art[ií]culo\s+de\s+la\s+subasta[^0-9]{0,40}(\d{1,6}(?:[,.]\d{1,2})?)\s*(?:€|EUR)?/iu);
    if (m?.[1]) return norm(m[1]);
  ...
  function extractWinner(text) {
    const s = String(text || "");
    let m = s.match(/^\s*([\p{L}\p{N}_.@\- ]{2,80}?)\s+ha\s+ganado\s*[:：-]\s*\d{1,6}(?:[,.]\d{1,2})?\s*(?:€|EUR)?/iu);
    if (m?.[1]) return norm(m[1]).replace(/^Actividad\s+/i, '').slice(0, 80);
    // Actividad TikTok: "Rosa Granado Arroyo ha ganado el artículo de la subasta 4 €"
    m = s.match(/(?:^|\|)\s*([\p{L}\p{N}_.@\- ]{2,80}?)\s+ha\s+ganado\s+el\s+art[ií]culo\s+de\s+la\s+subasta/iu) || s.match(/^\s*([\p{L}\p{N}_.@\- ]{2,80}?)\s+ha\s+ganado\s+el\s+art[ií]culo\s+de\s+la\s+subasta/iu);
    if (m?.[1]) return norm(m[1]).replace(/^Actividad\s+/i, '').slice(0, 80);
  ...
  function scanDom(reason = "tick") {
    if (isRevoked()) return;
    if (!/shop\.tiktok\.com\/streamer\//i.test(location.href)) return;
    if (scanning) return;
    scanning = true;
    try {
  ...
  // ═══════════════════════════════════════════════════════════
  // C4-F M3.1 — Crono Watcher (detecta fin de subasta)
  //
  // Observa el cronómetro "Finaliza dentro de XX:XX" en TikTok Live.
  // Cuando llega a 00:00 → dispara scan inmediato (con 3s de margen
  // para que TikTok genere el pedido del ganador).
  //
  // Selector: por texto "Finaliza dentro de" (robusto vs clases sc-XXX).
  // ═══════════════════════════════════════════════════════════
  const CRONO_TRIGGER_KEYWORDS = [
    "Finaliza dentro de",
    "Ends in",
    "结束于",        // ZH simplified (por si TikTok cambia idioma)
    "Termina en"     // variante ES alternativa
  ];
  ...
  class AuctionCronoWatcher {
    ...
    findCronoEl() {
      try {
        // Buscar todos los divs con texto del crono
        ...
        for (const div of divs) {
          ...
          // Dentro de este contenedor, buscar el span con formato MM:SS
    ...
    attachObserver(el) {
      ...
      this.attached = true;
      try {
        console.log("[EtiquetaLive Crono] Observer adjunto, valor inicial:", el.textContent);
      } catch (_) {}
      ...
      // También chequear el valor actual por si ya está en 00:00
      this.checkZero();
    }
    ...
    emitTick(txt) {
      ...
      if (txt === this.lastTickText) return;
      this.lastTickText = txt;
      const parts = txt.split(":").map(Number);
      const seconds = Math.max(0, (parts[0] * 60) + parts[1]);
      try {
        chrome.runtime.sendMessage({
          type: "EL_AUCTION_CRONO_TICK",
          seconds,
          totalSeconds: Math.max(60, seconds),
          label: txt,
          href: location.href,
          timestamp: Date.now()
    ...
    onZero() {
      if (isRevoked()) return;
      // Anti-rebote: solo 1 trigger cada 8 segundos
      const now = Date.now();
      if (now - this.lastTrigger < 8000) return;
      this.lastTrigger = now;

      const currentDelay = Math.max(1000, Math.min(30000, reconcileDelayMs));
      try {
        console.log("[EtiquetaLive Crono] 00:00 detectado → reconciliación única " + (currentDelay / 1000) + "s (config)");
      } catch (_) {}
      ...
      delays.forEach((delayMs, index) => {
        setTimeout(() => {
          ...
          try {
            console.log("[EtiquetaLive Crono] reconciliación intento", attempt, "delay", delayMs + "ms");
          } catch (_) {}
          try { scanDom("crono_zero_" + attempt); } catch (_) {}
          try {
            chrome.runtime.sendMessage({
              type: "EL_AUCTION_CRONO_ZERO",
              timestamp: Date.now(),
              href: location.href,
              attempt,
              delayMs,
              ...
              if (error) {
                try { console.warn("[EtiquetaLive Crono][diagnóstico] Background no respondió", error); } catch (_) {}
                return;
              }
              try { console.log("[EtiquetaLive Crono][diagnóstico] Respuesta background", response); } catch (_) {}
            });
          } catch (_) {}
        }, delayMs);
      });
  ...
  const start = () => {
    ...
    // Iniciar Crono Watcher en paralelo
    cronoWatcher.start();
  };
